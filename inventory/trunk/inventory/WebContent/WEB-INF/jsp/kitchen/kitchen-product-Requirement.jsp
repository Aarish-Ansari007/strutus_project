...
<html lang="zxx">
<head>
<title>Kitchen Product Requirements</title>
<!-- custom-theme -->
...
<script type="application/x-javascript"> 		
		...
		var tokenValue= '<%=request.getAttribute("TRACKERID") %>';
			...
			function getKitchenProductConsumptionList(){						
				document.getElementById("table").innerHTML = "<thead><tr><th>Id</th><th>Name</th><th>Date & Time</th><th>Unit Weight</th><th>Last Count</th><th>Requested</th><th>Quantity Type</th><th>Status</th><th>Supply Date</th><th>Left</th></tr></thead><logic:iterate id='formVOO' name='productList'><bean:define id='tempFormVOO' name='formVOO' type='com.indianmesh.inventory.kitchen.stock.ProductFormVO'></bean:define><tbody><tr id='<bean:write name='tempFormVOO' property='productId'></bean:write>'><td><span class='bt-content'><bean:write name='tempFormVOO' property='ID'></bean:write></span></td><td><span class='bt-content'><bean:write name='tempFormVOO' property='productName'></bean:write></span></td><td><span class='bt-content'><bean:write name='tempFormVOO' property='lastModifiedDate'></bean:write></span></td><td><span class='bt-content'><bean:write name='tempFormVOO' property='quantityPerUnit'></bean:write></span></td><td data-th='Qty Left in Stock'><span class='bt-content'><bean:write name='tempFormVOO' property='lastCount'></bean:write></span></td><td><span class='bt-content'><bean:write name='tempFormVOO' property='status'></bean:write></span></td><td><span class='bt-content'><bean:write name='tempFormVOO' property='supplyDate'></bean:write></span></td><td><span class='bt-content'><bean:write name='tempFormVOO' property='availableId'></bean:write></span></td></tr></tbody></logic:iterate>"; 
			...
			function addConsumptionInputFieldsInRow(){
				...
				for(var i=1;i<numberOfRows;i++){
				    ...
				    var newCell0 = currentRow.insertCell(5);					
						newCell0.align="center";
						newCell0.className="formlabel";
						
					var newCell1 = currentRow.insertCell(6);					
					...
					var newTextElement = document.createElement("INPUT");
						...
						newTextElement.name = "requirement"+(i-1);
						newTextElement.id = "requirement"+(i-1);
						newTextElement.maxlength="50"
						newTextElement.size="9";
					...
					var text = currentRow.cells[4].innerHTML;			
					...
					newSelectElement.addEventListener('change', function(){
						...
						if(numberOfRows==2){
							...
							var requirement = newTextElement.value;
							document.forms[0].action="kitchenRequirement.do?methodName=updateIndexkitchenRequirement&product="+product+"&available="+available+"&requirement="+requirement+"&type="+type;
			...
			function updateMultipleEntryRequirement(){
			    ...
			    if(len>2){								
					for(var i=0;i<len-1;i++){	
						var element = document.getElementById('requirement'+i).value;
						
						if(element == ''){
							alert("Please enter requirement quantity in all input fields displayed in list.");
							document.getElementById('requirement'+i).focus();
							return;
						}																																																    
					}						    	
					...
					for(var i=0;i<len-1;i++){	
						...
						for(var j=0; j<elements.length; j++){
							if(elements[j].selected && j==0){
						    	alert("Please select quantity type of all the requirement input fields.");
					...
					tempURI = tempURI + "&productId=" + productIdURI;
				 	...
				 	var urival = "";
					for(var i=0;i<len-1;i++){
						var val = document.getElementById('requirement'+i).value;
						if(val!=""){
					    	urival = urival + val + ",";
					 	}
					}
					
					tempURI = tempURI + "&requirementId=" + urival;
				...
				var status = window.confirm("Do you want to update the entered requirement details ?");
				if(status){
					document.forms[0].action="kitchenRequirement.do?methodName=updateIndexkitchenRequirement" + tempURI; 
					document.forms[0].submit();																		
				}						
			}		
			...
			function showListRecordsWise(){
				...
				document.forms[0].action="kitchenRequirement.do?methodName=getRequirementIndexMapByRecords&limit="+limit;
				document.forms[0].submit();
			}
			
			function getPagination(pageFrom,pageTo){																																
				var index=pageFrom+','+pageTo;																
				document.forms[0].action="kitchenRequirement.do?methodName=getRequirementIndexMapByPagination&index="+index;
				document.forms[0].submit();				
			}
			...
			function showListStatusWise(){
				...
				if( status=='All'){
					document.forms[0].action="kitchenRequirement.do?methodName=getIndexkitchenRequirement";  
					document.forms[0].submit();
				}else{					
					document.forms[0].action="kitchenRequirement.do?methodName=getRequirementsByStatus&status="+status;
					document.forms[0].submit();
				}
			}
			...
			function searchDetails(){				
				...
				if((start!="") && ( end !="")){
					document.forms[0].action="kitchenRequirement.do?methodName=getRequirementIndexMapByDate&start="+start+"&end="+end;
					document.forms[0].submit();
				}else{					
					if(search!=''){
						if(str.includes('KP')){
							...
							document.forms[0].action="kitchenRequirement.do?methodName=getRequirementIndexMapByProductId&productId="+search;
							document.forms[0].submit();
						}						
						else{
							document.forms[0].action="kitchenRequirement.do?methodName=getRequirementIndexMapByProductName&name="+str; 
							document.forms[0].submit(); 
						}
					}else{					
...
<body>
<form id="inventoryKitchenRequirementForm" method="post" name="inventoryKitchenRequirementForm" action="">
					  ...
					  <div class="col-md-12 listings">
										...
										<div class="col-md-2 col-sm-12">            
                                         ...
                                         <div class="col-md-6 col-sm-12 pull-right text-right">
											 <button type="button" class="btn btn-primary" onclick="updateMultipleEntryRequirement();">Check Updated Stock</button>
											 <button type="button" class="btn btn-danger" onclick="updateMultipleEntryRequirement();">Required</button>
											 <button type="button" class="btn btn-primary" onclick="updateMultipleEntryRequirement();">Save & publish</button>
											 <button type="button" class="btn btn-primary" onclick="updateMultipleEntryRequirement();">Save</button>										 
										</div>	
